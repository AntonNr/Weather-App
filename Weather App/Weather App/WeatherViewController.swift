...
class WeatherViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return weather?.hourly.temperature_2m.count ?? 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "WeatherTableViewCell", for: indexPath) as? WeatherTableViewCell {
            ...
            let currentTemperature = weather?.hourly.temperature_2m[indexPath.row]
            let currentWindSpeed = weather?.hourly.windspeed_10m[indexPath.row]
            let currentHumidity = weather?.hourly.relativehumidity_2m[indexPath.row]
